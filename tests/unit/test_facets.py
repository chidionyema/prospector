...
class TestBackfillFile:
    ...
    # "no entry names a pack that was never published" is now scripts/store_audit.py
    # (BACKFILL_ENTRIES). It answers itself by reading store/dossiers/, which is gitignored
    # (.gitignore:43) and therefore empty in CI — so as a test it could only ever say "there
    # is no catalogue here" while phrasing it as "phantom backfill entry". The checks below
    # need nothing but the backfill file, which IS committed, so they hold on every checkout.
