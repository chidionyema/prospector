...
_OK_BODY = b'{"choices":[{"message":{"content":"{\\"ok\\": true}"}}],"usage":{}}'
...
def test_a_429_is_retried_and_the_call_still_succeeds(monkeypatch):
    ...
    def fake(req, timeout=None, total_deadline=None):
        calls["n"] += 1
        if calls["n"] <= 2:
            raise OSError("HTTP Error 429: Too Many Requests")
        return _OK_BODY

    monkeypatch.setattr(op_mod, "_urlopen_read_bounded", fake)
...
def test_the_delay_actually_grows(monkeypatch):
    ...
    monkeypatch.setattr(op_mod, "_urlopen_read_bounded",
