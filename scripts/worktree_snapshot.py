...
def git(args: list[str], cwd: Path, env: dict | None = None, timeout: int = 300) -> tuple[int, str]:
    ...
    # non-empty. Measured 2026-08-19: `git push` of 17 snapshot refs was REJECTED, the pre-push
    ...
    # was meant to protect. See memory `a-step-that-discards-the-stderr-explaining-its-own-death`.
    if p.returncode == 0:
