...
#
# Raised 8000 -> 14000 on 2026-08-20. The cap was set when the trimmed block was ~3700 chars and
# held three laws. The founder has since written seven plus an ordering preamble: measured that
# day, `_rules_only` returns 11,820 chars, so at 8000 the block dropped LAW 5, LAW 6 and LAW 7 --
# the last three written, including the one that closes an incident. 14000 fits all seven with
# room for roughly one more before the truncation notice fires again.
LAWS_MAX_CHARS = 14000

# A header above the first law is allowed, but a header longer than a law is not a header. Over
# this it is dropped and the laws ship without it, so an unbounded preamble can never eat the
# budget the laws need. Measured 2026-08-20: the real preamble is 1377 chars.
PREAMBLE_MAX_CHARS = 2500
...
def _declares_a_law(head):
    """True when this block IS the laws: it carries at least one `# LAW ` heading of its own."""
    return any(line.startswith("# LAW ") for line in head.split("\n"))
...
def read_laws():
    ...
    #
    # THE TRAP THIS CLOSES. This test used to be `head.startswith("# LAW")`. On 2026-08-20 the
    # founder put an ordering table above LAW 1, so the file began `# THE ORDER OF THE LAWS` and
    # this function returned None from that edit onward: the hook injected nothing, and said
    # nothing about it. Nothing broke loudly, because the laws still arrive in the `claudeMd`
    # block Claude Code supplies on every request -- the hook had simply stopped contributing and
    # no signal existed to say so. A guard pinned to one spelling of a heading fails the day the
    # heading is renamed, which is the same class as `a-guard-pinned-to-one-syntax...`.
    #
    # So test what the block IS -- it declares laws -- rather than what it starts with. A preamble
    # is allowed and bounded, which is how the original 40KB concern stays answered.
    if not _declares_a_law(head):
        return None
    if "\n# LAW " in head and len(head.split("\n# LAW ", 1)[0]) > PREAMBLE_MAX_CHARS:
        head = "# LAW " + head.split("\n# LAW ", 1)[1]
...
def selftest():
    ...
    try:
        ...
        try:
            ...
            # A preamble ABOVE the first law is legal and must not silence the whole block. This
            # is the exact shape that stopped the injection dead on 2026-08-20.
            with open(g["LAWS_FILE"], "w") as fh:
                fh.write("# THE ORDER OF THE LAWS\n\nlower number wins\n\n"
                         "# LAW 1 - FIRST\n\nbody\n\n---\n\nrest\n")
            got = read_laws()
            check("a preamble above LAW 1 does not silence the laws",
                  got is not None and "# LAW 1 - FIRST" in got, True)
            check("the preamble travels with the laws",
                  got is not None and got.startswith("# THE ORDER OF THE LAWS"), True)

            # ...but an UNBOUNDED preamble is dropped rather than allowed to eat the budget.
            with open(g["LAWS_FILE"], "w") as fh:
                fh.write("# NOTES\n\n" + ("z" * 4000)
                         + "\n\n# LAW 1 - FIRST\n\nbody\n\n---\n\nrest\n")
            got = read_laws()
            check("an oversized preamble is dropped, the laws still ship",
                  got is not None and got.startswith("# LAW 1 - FIRST") and "zzz" not in got, True)
            # The filler is sized FROM the cap, never pinned to a number. These two cases were
            # written with a literal 9000 when the cap was 8000; raising the cap to 14000 on
            # 2026-08-20 made them stop testing anything and they failed as if the guard had
            # broken. A fixture that hardcodes the value it is measuring against grades the
            # constant, not the behaviour.
            over = LAWS_MAX_CHARS + 1000
            with open(g["LAWS_FILE"], "w") as fh:
                fh.write("# LAW 0\n\n" + ("x" * over) + "\n\n---\n\nrest\n")
