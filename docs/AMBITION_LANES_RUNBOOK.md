...
Everything lives in **`config.yaml` lines ~223-243**. No code change, no restart of anything but
the daemon.
...
`schedule.batch_size` (currently 15, `config.yaml` ~line 852). `run.py::_lane_counts` distributes
...
- **`generation.operator_archetype`** (~line 632) — the default when no lane is engaged. Lane
  blocks override it. Editing this alone does nothing while lanes are active.
- **`generation.archetypes`** (~line 685) — the actual prompt text (`binding` / `forbid`) injected
...
- **`profiles`** (~line 700) — reusable bundles of generation overrides, selectable with
