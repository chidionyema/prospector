...
| 2026-08-16 | stranded packs | `ops/automations/stranded_packs.py` | `ops/config/stranded_packs.yaml` | `docs/RUNBOOKS.md` | **auto** | Answers ENG-1 as a measurement instead of a remembered number: 38 of 100 passed packs cannot be sold — 29 failed the content lint, 9 were never linted. Names the blocking rule per pack, so the output is a work list. No `--fix` on purpose: repair costs model calls (R8). |
