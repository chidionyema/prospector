...
| Edge routing | **Gateway API**, implemented by **Traefik** | Gateway API is a Kubernetes SIG-Network project, v1.6.1 (2026-07-16); Traefik MIT, 64,556 stars, v3.7.11 (2026-08-19) | Apache-2.0 / MIT | The obvious rival is the Ingress API with ingress-nginx, and it is **retired**. Measured 2026-08-24: `gh api repos/kubernetes/ingress-nginx --jq .archived` returns `true`, last push 2026-03-23. The Kubernetes Steering and Security Response Committees, 2026-01-29: *"There will be no more releases for bug fixes, security patches, or any updates of any kind after the project is retired"*, and they name Gateway API as the path. Its planned successor InGate never reached maturity. Traefik is not a preference — k3s already ships it, so the controller is installed and maintained by the distribution. Gateway API is what makes that reversible: `gatewayClassName` is the only controller-specific string in `deploy/k8s/base/edge.yaml`, and **Envoy Gateway** (Apache-2.0, 2,975 stars, v1.9.0 on 2026-08-15) is the named replacement, one word away. |
| TLS certificates | **cert-manager** | CNCF Graduated, CLOMonitor 95.65 | Apache-2.0 | Nothing else is close. Caddy does this today inside the edge container and keeps doing it under `docker compose`, which is right on a host with no cluster; on the cluster, cert-manager moves it to where the rest of the config lives. The install detail that is easy to miss and silent when missed: cert-manager.io/docs/usage/gateway/ says *"Since cert-manager 1.15, the Gateway API support is no longer gated behind a feature flag, but you still need to enable the Gateway API support"* — the Helm release needs `config.gatewayAPI.enabled: true`, and without it a Gateway simply never gets a certificate. |
