...
|---|---|
| Reorder the verdict chain so claude_cli leads | **Already the config.** `config.yaml:53` reads `[claude_cli, standardcompute, minimax]` with a comment stating it is a tail, not a head. MiniMax ruled 24 of 30 checks on 2026-08-13 because claude_cli was walled and standardcompute was dead-marked (`provider_health.json`: strikes 4, *"You've used up your free usage"*). The chain fell through exactly as designed. |
