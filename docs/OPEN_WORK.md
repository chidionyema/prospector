...
| B5 | `store/golden_runs` is cloned into every worktree, so the headline gate score answers to cwd | `scripts/setup_worktree.sh:186`, `prospector/ops/readers.py:899` | **OPEN, measured 2026-08-20.** Seven scratchpad worktrees hold the SAME 77 byte-identical records, newest 2026-08-15 (5.42 days old). Both clones' canonical stores have no `golden_runs` directory at all. `readers.latest_golden()` is the estate's headline discrimination, so it answers **100.0%** or **nothing** purely on which directory the reader started in. Same class as the store-path-from-`__file__` trap in CLAUDE.md. `_audit_path` mkdirs, so a missing directory is NOT the cause — the cause is the CoW clone at `setup_worktree.sh:186` plus the main checkout's copy having been deleted. Not fixed: wt-storeroot-1e and wt-storeroot-4a hold that file. What IS fixed is `tools/engine_baseline.py::axis_a4`, which now reports `store`, `measured_at`, `age_days` and `config_hash` beside the value. Memory: `the-golden-score-answers-to-cwd.md`. |
| B6 | `e2e/mobile.spec.ts` grades 10 of 23 nav screens | `e2e/session.ts` `SCREENS` | **OPEN, owned by wt-storeroot-42, sequenced after their `/reports` page lands.** `SCREENS` is a hand-kept list of ten whose docstring claims it tracks the nav; the nav has 23 destinations, so 13 are ungraded at both widths and a screen added to the nav silently leaves the guard. Cost of deriving it from `GROUPS`: measured 42.9s for 11 screens across 390px and 320px, so 23 is ~90s against a 180s timeout, 2x margin. Deliberately deferred under LAW 1 — putting 12 never-graded screens under the guard in one commit can turn a branch red that is carrying a live founder ask. |
