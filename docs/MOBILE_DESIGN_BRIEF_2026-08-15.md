...
| 5 | Horizontal overflow in free-sample section | **NOT REPRODUCED in source** | Swept `sample.tsx` and the components it renders for fixed widths, `w-max`, `min-w-[…]` and px/rem grid tracks: none. Everything is `max-w-[…ch]` or `minmax(0,1fr)`. Possibly fixed by #217 (`a94dc54`), which landed concurrently with the brief. **Needs a browser measurement at 390 to close** — source is not proof of layout (memory: `never-judge-design-by-grepping-html`). The `w-max` sector rail that overflows at 320 is scroll-snap and is BY DESIGN; not this. |
...
| 7 | Spacing scale 8/16/24/40/64, cap section gaps | **PARTIAL — measured, not swept** | Distinct vertical steps in `index.tsx`: on-scale `mt-2/4/6/10/16`, `py-16/24`; **off-scale** `mt-3, mt-5, mt-7, mt-8, mt-9, mt-12, mt-14, mt-20, mb-3, mb-8, py-3, py-12, py-14, py-20`. Largest single utility is `py-24` (96px) at the section band (`blocks.tsx:198` `py-16 md:py-24`), so **no single gap is near the ~300px the brief reports** — that figure can only come from a band's padding stacking with an adjacent margin, which source cannot settle. Not swept: renormalising 14 steps across the busiest page is a visual change with no test behind it and no before-measurement. **Needs the 390/320 Playwright pass first.** |
