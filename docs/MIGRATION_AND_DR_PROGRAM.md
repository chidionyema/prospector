...
| F-39 | **THE WHOLE STACK MOVES, not compute alone.** Every one of the ten planes has a deliverable on the target substrate — state, secrets, identity and DNS, observability, jobs, the money path, delivery, control and the knowledge base — with no Fly-specific and no laptop-specific piece left behind. Founder, 2026-08-21: *"the whole stack"* | the ten-plane bring-up: **each plane's own drill run on the new substrate**, not merely a pod that starts. A move where the engine serves and the jobs, the alerts or the money path did not come with it is a failed move, not a partial one. **Started 2026-08-24, and the fraction is small enough to state exactly: four workloads of one plane are declared.** `deploy/k8s/base/engine.yaml` replaces the Deployment `deploy/targets/k8s.sh` wrote inline at `t_start`. Why it was written at all: the adapter's version was graded against this estate's own 26 admission policies and **refused by ten of them** (`disallow-latest-tag`, `disallow-privilege-escalation`, `drop-all-capabilities`, `require-pod-probes`, `require-requests-limits`, `require-ro-rootfs`, `require-run-as-nonroot`, `restrict-seccomp-strict`, `secrets-not-from-env-vars` — `pass: 19, fail: 10`), so the only Kubernetes deploy path the estate had would not have been admitted to the cluster the estate is building. The replacement grades `pass: 31, fail: 0` and CI gate 6 regrades it in both overlay builds, refusing a vacuous pass. **The store API followed the same day**: `deploy/k8s/base/api.yaml`, the money rail, so one replica, `Recreate`, ReadWriteOnce and the `single-writer` label are enforced at admission rather than left to whoever edits the file next. It needed the same secrets-as-files answer the engine needed, and got it from the framework rather than from a new script: `Microsoft.Extensions.Configuration.KeyPerFile`, already in `Microsoft.AspNetCore.App`, maps a file named `Jwt__SigningKeyPem` onto the key `Jwt:SigningKeyPem` the C# already reads, so no package was added and no resolver was written (LAW 43). `Store.Api/Infrastructure/FileSecrets.cs` adds only the refusal on a missing or empty mount and reads the same `PROSPECTOR_SECRETS_DIR` the engine reads. **The storefront followed the same day**: `deploy/k8s/base/web.yaml`, and it is the one workload of the three that owns no state, so it is the only one that runs two replicas, rolls rather than Recreates, and gets a PodDisruptionBudget. The engine and the API deliberately get none: `minAvailable: 1` on a `replicas: 1` workload blocks a drain forever rather than protecting anything, which LAW 38 grades as an outage. It mounts no Secret because every `NEXT_PUBLIC_*` value is a build argument baked in at `docker build` — which is what `NEXT_PUBLIC_` means — and the consequence is that staging and production storefronts are DIFFERENT IMAGES and cannot share a tag. **The edge followed, and it is the one manifest here that is NOT a port.** `deploy/compose/docker-compose.yml` runs `caddy:2.10-alpine` behind a hand-written Caddyfile, and lifting that container into a Deployment was the smallest diff and the wrong one, so the research ran before the first line (LAW 43, R6) and is on the record in `~/dev/code/crew/science/RESEARCH-LEDGER.jsonl`. It found the Ingress API's flagship controller archived — `gh api repos/kubernetes/ingress-nginx --jq .archived` returns `true`, last push 2026-03-23 — with the Kubernetes Steering and Security Response Committees writing on 2026-01-29 that *"There will be no more releases for bug fixes, security patches, or any updates of any kind after the project is retired"* and naming Gateway API as the migration path; its planned successor InGate never reached maturity. So `deploy/k8s/base/edge.yaml` is a Gateway, three HTTPRoutes and a cert-manager ClusterIssuer, with `gatewayClassName` as the only controller-specific string in the file — k3s ships Traefik (MIT, v3.7.11, 2026-08-19) and Envoy Gateway is one word away. Compose keeps Caddy unchanged, because on a Docker host with no cluster it is the right answer. **Measured 2026-08-24 with the pinned v1.16.1 CLI: both overlays grade `87 rules, pass: 91, fail: 0` across 15 workload documents, gate 5 holds at 13/13 assertions, 8 tests in `Store.Tests/Infrastructure/FileSecretsTests.cs` pass, and the 12 tests in `tests/unit/test_the_k8s_gate_cannot_pass_on_nothing.py` pass with the gate's document floor raised from 4 to the measured 15 — a floor left at the day-one number would have admitted a build that had silently lost three of the four workloads** — the four refusals plus the four things the manifest bets on, including that a Kubernetes projected-volume symlink farm yields the keys and none of the `..`-prefixed machinery. The engine reads the file-mounted secrets `secrets-not-from-env-vars` forces: `prospector/file_secrets.py`, called from `prospector/__init__.py` before any module in the package runs, so the 30 files that read a credential from `os.environ` needed no change, and it refuses to start on a partial mount. **The fifth and last compose service is now answered, and the answer is that nothing goes into `deploy/k8s/base` for it.** The CI runner was measured before a line was written: `gh api repos/chidionyema/prospector/actions/runners --jq .total_count` returns **0**, and all four repo variables that select a runner — `CI_RUNS_ON`, `CI_LIGHT_RUNS_ON`, `CI_HEAVY_RUNS_ON`, `CI_DEPLOY_RUNS_ON` — are `ubuntu-latest`. Zero self-hosted runners registered, every workflow on GitHub-hosted, and the compose service behind a `ci` profile that is not in the default stack. Porting `deploy/runner/entrypoint.sh`'s 191 lines of ephemeral-runner bash into a Deployment would have been building infrastructure for a service with no instances and no jobs, which is the LAW 43 mistake in its most expensive form: hand-rolled code carried forward onto a new substrate because it was there. If capacity ever argues for self-hosted again the artifact is `actions/actions-runner-controller`'s `gha-runner-scale-set` Helm chart (Apache-2.0, 6,459 stars, 0.14.2, archived=false) behind an Argo CD Application — never a hand-written Deployment — and the switch costs one repo variable's value, because every workflow's `runs-on` already reads one. Recorded on the crew research ledger, crew PR #144. **Still ○ underneath — this row previously said the remaining services were "generated inline by `deploy/targets/k8s.sh`", and they were not; that script emits three documents, all for the engine, and names no other service. Nine planes have nothing. No cluster has applied any of it, and the edge additionally needs two components no cluster here has — the Gateway API CRDs, which k3s does not install, and cert-manager with `config.gatewayAPI.enabled: true`, without which a Gateway silently never gets a certificate. The 91 passes are also 91 with the edge added and 91 without it: no policy in the set matches a Gateway, an HTTPRoute or a ClusterIssuer, so those five documents are counted, not graded. Neither the API image nor the storefront image has ever been run under `runAsUser 10001` with a read-only root filesystem and neither Dockerfile carries a `USER` line.** **One of those open items closed the same day, and the correction is worth stating because this row carried the wrong answer for a morning: the ops console DOES serve under `runAsUser 10001` with a read-only root.** The first drill ran `next start` from a shell inside the container and saw an empty log, nothing listening and no process, and the honest reading of that was "not proved". Re-run with node as the container's PID 1 — `--user 10001:10001 --read-only`, tmpfs at `/tmp` and `.next/cache`, `HOME=/tmp`, against a freshly built image — printed `Next.js 16.3.1`, `Ready in 2.7s`, `Running=true ExitCode=0`, and `GET /` on the published port returned **307**; a control arm of the same image and command with no `--user` and no `--read-only` returned **307** in 2.1s, so the status is what the app answers and not a symptom of the constraints (LAW 29), and a Kubernetes `httpGet` probe passes on 200–399. **What is still open is one line, not a mystery.** The pod runs the image's own entrypoint, so supervisord starts the console; drilled under the same securityContext it exits 2 with `Error: Can't drop privilege as nonroot user`, which is `deploy/engine/supervisord.conf`'s `user=root`, measured twice now. Dropping it means the image also runs as 10001 under `docker compose`, where the bind mount `./data:/data` is owned by the host user and not by 10001 — that ownership decision is the whole remaining cost, and it is a decision rather than a discovery | D-P1.5 | 6 | ◐ |
...
| F-47 | **The substrate refuses work that breaks the estate's standards, and admits work that does not.** A fence proven only in the refusing direction has never been shown safe to install — LAW 38 grades a guard that blocks correct work as an outage, not a false positive | `deploy/rehearse_cluster.sh policy`: the refusals the policy set must make and the admissions it must not block, both halves in the same run, and the run fails if either half fails. **Now 26 policies, not 7** — see F-51. **The offline half is now proved and is F-55; this row stays `◐` because no live API server has ever run it** | D-P1.9 | 1 | ◐ |
| F-55 | **THE POLICIES DECIDE, AND THE DECISION IS GRADED WITHOUT A CLUSTER.** Everything F-52 gates proves the policy set BUILDS. A policy that builds is a policy nobody has watched make a decision. The Kyverno CLI runs the same admission engine offline, so the verdict comes from the code that will make it in production rather than from a reading of the YAML. This is the cheap one of LAW 15's two angles and it removes the cluster from the critical path of ever knowing whether a standard works | `.github/workflows/k8s-manifests.yml` gate 5, against 11 fixtures each carrying exactly one violation: 13 assertions pairing resource to the specific policy AND rule that must refuse it, plus `deploy/k8s/policy-tests/admitted.yaml` which must be ADMITTED with zero failures. **Measured 2026-08-24: 13/13, and 31 rule evaluations passed out of 87 rules loaded on the reference workload.** Proved both ways — the suite fails when a policy is dropped from the set, when a fixture stops violating, and when the reference workload is made non-compliant. **The gate reads the CLI's `REASON` field and never its exit code**: measured the same day, `kyverno test` v1.16.1 printed `Test Summary: 13 tests passed and 0 tests failed` and exited 0 with an assertion violated, while its own `REASON` column said `Want fail, got pass` | D-P1.9 | 1 | ✅ |
| F-56 | **THE POLICY SET IS ON AN API ITS OWN PROJECT HAS DATED FOR REMOVAL, and it moves before that date rather than on it.** F-51 bought the estate 24 upstream policies; every one of them is a `kyverno.io/v1` `ClusterPolicy`, which is the type Kyverno deprecated in v1.17 and removes in v1.20. A standard that stops being enforced because the enforcer dropped the API is the same outage as never having written it, and the cost grows with the policy surface, so this moves BEFORE the shop, api, web and edge manifests land rather than after. The replacement is `ValidatingPolicy`, CEL rather than JMESPath, which compiles to the Kubernetes-native ValidatingAdmissionPolicy | `kubectl kustomize deploy/k8s/overlays/production \| grep -c 'kind: ClusterPolicy'` returns 0, the same 13 refusal assertions and the admitted fixture still pass in `.github/workflows/k8s-manifests.yml` gate 5, and the CI gate refuses a build that reintroduces the legacy kind. **Researched and confirmed against the source 2026-08-24, not taken on report: Kyverno's own docs state ClusterPolicy and Policy are deprecated and removed in v1.20, with v1.19 the last release carrying full support, and publish a field-by-field migration guide. This repo pins v1.16.1, so nothing is broken today and there is a whole release of headroom** | D-P1.9 | 3 | ○ |
