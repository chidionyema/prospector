...
def render():
    ...
    decision_filter = col1.selectbox("Decision", decisions)
    lanes = ["all"] + sorted({r.get("ambition_tier") or "" for r in rows if r.get("ambition_tier")})
    lane_filter = col2.selectbox("Lane", lanes)
    structural_forms = ["all"] + sorted({r.get("structural_form") or "" for r in rows if r.get("structural_form")})
    form_filter = col3.selectbox("Form", structural_forms)
    search = col4.text_input("Search title", "").lower()

    filtered = rows
    if decision_filter != "all":
        filtered = [r for r in filtered if (r.get("decision") or "").lower() == decision_filter]
    if lane_filter != "all":
        filtered = [r for r in filtered if r.get("ambition_tier") == lane_filter]
    if form_filter != "all":
