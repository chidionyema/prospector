...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      // `/collections` is back to `/ideas` (founder, 2026-08-18: "nav reverted to Categories ->
      // /ideas"). The redirect direction flips with it. Both rules are 308: the move is
      // permanent, and a 307 would ask every crawler to keep re-checking a page that is never
      // coming back.
      //
      // The slug rule must come FIRST. Next.js takes the first match, and `/collections` with no
      // `:slug` would otherwise be tried against `/collections/evenings` before the specific rule
      // is reached. Two rules rather than one `/collections/:slug*`, because the index needs its
