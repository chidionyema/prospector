...
describe('Design contract — catalogue blueprint cards (pages/index.tsx)', () => {
  ...
  it('renders cards with white bg, 1px border, 8px radius, 24px padding', () => {
    // The PackCard link element
    assertContains('card white bg', page, 'bg-white');
    assertContains('card 1px border', page, 'border border-border');
    // 8px radius = rounded-lg (the design token --radius-lg is 8px)
    assertContains('card 8px radius', page, 'rounded-lg');
    // 24px padding = p-6
    assertContains('card 24px padding', page, 'p-6');
  });
