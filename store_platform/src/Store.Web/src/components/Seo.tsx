...
export function Seo({
  ...
  // The 1200x630 link-preview card. The site-wide default is `DEFAULT_OG_IMAGE_PATH`
  // (`public/og.png`, regenerated by `scripts/gen-brand-assets.mjs`); a page with its own card
  // passes `ogImagePath` and wins. Needs an absolute URL, so like the canonical it only emits on a
  // configured build.
  //
  // The path moved to `lib/seo/ogImage.ts` on 2026-08-14 rather than staying a literal here: it
  // was hardcoded in this file AND in `schema.ts`'s Product `image`, which is the drift that
  // module was written to prevent, and the default now carries a `?v=` cache-bust that both call
  // sites must share to be worth anything. The old comment credited `scripts/gen-og-image.mjs`,
  // which has never existed in this repo -- there was no generator at all, which is how the card
  // stayed another company's for two months.
  const ogImage = SITE_URL ? `${SITE_URL}${ogImagePath ?? DEFAULT_OG_IMAGE_PATH}` : undefined;
