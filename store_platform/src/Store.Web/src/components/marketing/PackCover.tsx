...
export default function PackCover({ pack, variant, asLink = false, className }: PackCoverProps) {
  ...
  const content = (
   ...
   <div className="absolute left-3 top-3 right-3 flex items-start justify-between gap-2 text-caption font-bold uppercase tracking-[0.2em] text-white/85">
        <span className="truncate">№ {pack.id.slice(0, 12)}</span>
        <span className="shrink-0 rounded-md bg-black/20 px-1.5 py-0.5 backdrop-blur-sm">
      ...
      <div className="absolute inset-x-0 bottom-0 px-4 pb-4 pt-12 bg-gradient-to-t from-black/55 via-black/25 to-transparent">
        <p className="font-serif text-meta font-bold leading-[1.15] text-white line-clamp-3">
          {pack.title}
        </p>
        {pack.cardLine && variant === 'hero' && (
          <p className="mt-2 text-caption leading-snug text-white/85 line-clamp-2 font-medium">
            {pack.cardLine}
          </p>
        )}
     ...
     <p className="mt-2 text-caption font-bold uppercase tracking-[0.2em] text-white/75">
            {cat.label}
          </p>
        )}
      </div>
