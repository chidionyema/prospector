...
/*
  The verification dossier, in the hero, at one glance.

  WHY IT MOVED. The full `DossierPreview` sits roughly 80% down this page. It is the single most
  ...
  WHY THIS IS NOT A SECOND COPY OF `DossierPreview`. Two renderings of the same eight rows on one
  page is the "same paragraph four times" defect this pass exists to remove. So the strip and the
  panel are deliberately different OBJECTS with different jobs: this is the SHAPE of a dossier
...
const DOMAINS = Array.from(
  new Set(checks.flatMap((check) => check.sources.map((source) => domainOf(source.url)))),
).filter(Boolean);
...
export function HeroDossier({ className }: { className?: string }) {
  return (
    <div className={cx('max-w-[46rem]', className)}>
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-2">
        {/* The eight verdicts as eight marks, in the order the checks ran. This is the whole
            dossier compressed to a single glyph run: a reader takes in "eight checks, one of them
      ...
      {/* The kill total lives HERE, attached to the dossier, because the two facts only mean
