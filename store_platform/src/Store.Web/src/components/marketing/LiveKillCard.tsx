  ...
  return (
    <div
      className={cx(
        ...
        'font-mono shadow-[3px_3px_0_var(--text)]',
        className,
      )}
    >
      {/* Header. Labelled as what it is, a snapshot of the audit trail, rather than "LIVE". */}
      <div className="flex items-center justify-between gap-3 border-b border-bg/20 px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest">
        ...
        <div className="mt-4 flex flex-wrap items-end justify-between gap-x-4 gap-y-3 border-t border-bg/20 pt-3">
          <div>
            ...
            <div className="mt-1 flex items-baseline gap-3">
              <span className="text-base font-black text-on-band-danger">{killed.toLocaleString('en-GB')}</span>
              <span className="text-[10px] uppercase text-bg/60">killed</span>
              <span className="text-base font-black text-on-band-success">{passed.toLocaleString('en-GB')}</span>
              <span className="text-[10px] uppercase text-bg/60">survived</span>
            </div>
          </div>
          <Link
            href="/kill-log"
