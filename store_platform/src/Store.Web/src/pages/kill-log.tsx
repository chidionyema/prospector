  ...
  return (
    <MarketingLayout>
      ...
      <SectionBand bg="white" width="6xl" className="pt-14 pb-8 md:pt-20 md:pb-10 text-center">
        ...
        <h1 className="mx-auto max-w-[22ch] text-balance text-4xl font-bold leading-[1.08] tracking-tight text-text md:text-5xl">
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <h2 className="text-base font-bold leading-snug text-text/60 line-through decoration-warning/60 decoration-2">
                    {entry.title}
                  </h2>
                  {entry.oneLiner && (
                    <p className="mt-1.5 max-w-[70ch] text-sm leading-relaxed text-muted">{entry.oneLiner}</p>
                  )}
                </div>
                <span className="inline-flex flex-none items-center gap-1.5 rounded-full bg-warning/10 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-warning">
              ...
              <div className="mt-4 border-t border-border/70 pt-4">
                ...
                <p className="mt-2 max-w-[72ch] text-sm leading-relaxed text-text/80">{entry.reason}</p>
              </div>
              ...
              {(entry.citations.length > 0 || entry.date) && (
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  {entry.date && (
                    <span className="ml-auto font-mono text-[11px] text-muted">{formatDate(entry.date)}</span>
                  )}
                </div>
              )}
            </li>
