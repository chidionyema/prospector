    ...
    <MarketingLayout>
      ...
      >
        <div className="space-y-6 mt-12 md:mt-16">
          {FAQS.filter((item) => item.category === 'packs').map((item, i) => (
            <div key={i} className="bg-surface border border-border p-5 transition-all group">
              <h2 className="text-lg font-black text-text mb-4 leading-tight group-hover:text-primary transition-colors">{item.question}</h2>
      ...
      <Section bg="white" width="7xl">
        <aside className="space-y-6 max-w-sm mx-auto">
          <div className="bg-surface border border-border p-6 transition-colors group">
