...
/**
 * The real per-pack check counts, read out of `qaVerdictSummary`.
 *
 * WHY THIS EXISTS
 *
 * The pack page, the catalogue card and the mini card all printed a hardcoded `6/6 checks`.
 * Measured against the live API on 2026-08-06, that is false for 21 of the 61 listed packs:
 * 40 are `6/6`, 14 are `8/8`, 3 are `7/8`, 3 are `9/9` and 1 is `6/8`. The count is
 * lane-dependent -- the side-hustle lanes run `buyer_intent`, `currency`, `route_to_market`
 * and `claims_verifiable` instead of `value_durability` and `incumbency`. On the four packs at
 * `7/8` and `6/8` the hardcoded line also OVERSTATED, claiming everything cleared when one or
 * two did not. "No unsourced numbers ship, ever" applies hardest to the line sitting next to a
 * price, so the number now comes from the pack.
 *
 * The input is machine-generated by `prospector/bridge.py::_trust_fields` (line 175) from one
 * f-string, so the regex is anchored to that exact template rather than being permissive. A
 * string that does not match yields `null` and the caller renders NO check claim at all -- an
 * unparsed summary must never fall back to a guessed number, which is the defect being removed.
 */
export function parseCheckCounts(
  summary?: string,
): { cleared: number; total: number } | null {
  const m = /^(\d+)\/(\d+) checks cleared · (\d+) sources cited$/.exec((summary ?? '').trim());
  if (!m) return null;
  const cleared = Number(m[1]);
  const total = Number(m[2]);
  if (!Number.isFinite(cleared) || !Number.isFinite(total) || total <= 0 || cleared > total) {
    return null;
  }
...
/**
 * The buyer-visible prose on a pack, in the site's voice.
 *
 * `lib/text.ts` has exported `nodash()` since the kill-log work, `dashFree.test.ts` bans em and en
 * dashes across every file in the repo as "the most universally recognised AI writing signature",
 * and the engine's own publish tools run the identical Python `nodash()` before writing. None of
 * that reached the catalogue, because the catalogue arrives over HTTP rather than out of the repo,
 * and NOTHING in the storefront called `nodash` at all -- it was an exported function with zero
 * consumers. Measured against the live API on 2026-08-06:
 *
 *   curl -s https://api.mumchimp.com/catalog | python3 -c "import sys,json;d=json.load(sys.stdin);\
 *   print(sum(1 for i in d if any(c in i['title'] for c in '--')), 'of', len(d))"
 *   -> 52 of 61 titles carry an en or em dash
 *
 * So the character the site bans everywhere was set in the largest type on the money page, 52 times
 * out of 61 ("IEPBlueprint <en dash> the parent's tool...", mobile-pack-fold.png). The boundary is
 * the right place to fix it, for the same reason markdown is converted here rather than at the
 * source: the engine's dossier is the record of what was verified and must not be rewritten to suit
 * a font. Only prose fields are touched -- never ids, prices, dates or facet keys.
 */
const PROSE_FIELDS = [
  'title',
  'oneLine',
...
function inTheSiteVoice<T extends Pack>(pack: T): T {
  const out = { ...pack } as unknown as Record<string, unknown>;
  for (const field of PROSE_FIELDS) {
    const value = out[field];
    if (typeof value === 'string') out[field] = nodash(value);
  }
  // Arrays of prose. `whatYouGet` and `sampleExtract` are rendered verbatim on the pack page.
  for (const field of ['whatYouGet', 'sampleExtract']) {
    const value = out[field];
    if (Array.isArray(value)) {
      out[field] = value.map((v) => (typeof v === 'string' ? nodash(v) : v));
    }
  }
  return out as T;
}
