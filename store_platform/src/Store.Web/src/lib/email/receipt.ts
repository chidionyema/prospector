...
export function renderReceiptText(order: ReceiptOrder, brand: string): string {
  return [
    ...
    `  ${order.orderPath}`,
    '',
    'The voice is source-or-die. Sourced, not sold. Refutational, not promotional. Every claim in your pack cites a retrievable source. The QA report inside the pack lists every source, and the kill log lists the ideas that did not survive the checks.',
    '',
