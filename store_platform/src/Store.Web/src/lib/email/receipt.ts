...
export function renderReceiptText(order: ReceiptOrder, brand: string): string {
  return [
    ...
    `  ${order.orderPath}`,
    '',
    'The voice is source-or-die. Sourced, not sold. Refutational, not promotional. Every claim in your pack cites a retrievable source. The QA report inside the pack lists every source, and the kill log lists the ideas that did not survive the filter.',
    '',
...
export function renderReceiptHtml(order: ReceiptOrder, brand: string): string {
  ...
  return [
    ...
    `<p style="margin:0 0 24px 0;font-size:11px;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:#6B6B6B;">${safeBrand}</p>`,
    ...
    `<p style="margin:0 0 24px 0;font-size:16px;color:#0A0A0A;">${safePackTitle} is yours. Every claim in it cites a retrievable source. The QA report inside the pack lists every source, and the <a href="${safeBrand === 'Mumchimp' ? 'https://mumchimp.com/kill-log' : '/kill-log'}" style="color:#0A0A0A;text-decoration:underline;">kill log</a> lists the ${'{count}'} ideas that did not survive the filter.</p>`,
    '<div style="margin:0 0 24px 0;padding:16px;border:1px solid #E5E5E5;background-color:#F7F7F5;">',
    '<p style="margin:0 0 8px 0;font-size:11px;font-weight:700;letter-spacing:0.2em;text-transform:uppercase;color:#6B6B6B;">Permanent access link</p>',
