...
export function renderReceiptText(order: ReceiptOrder, brand: string): string {
  return [
    ...
    `  ${order.orderPath}`,
    '',
    'Every claim in your pack cites a retrievable source. The QA report lists every source. The rejected list names the ideas that did not survive the research checks.',
    '',
    ...
    '',
    `Read how the research works on the how-it-works page.`,
    '',
    `Mumchimp`,
  ].join('\n');
}
...
export function renderReceiptHtml(order: ReceiptOrder, brand: string): string {
  ...
  return [
    ...
    `<p style="margin:0 0 24px 0;font-size:16px;color:#0A0A0A;">${safePackTitle} is yours. Every claim in it cites a retrievable source. The QA report lists every source, and the <a href="${rejectedUrl}" style="color:#0A0A0A;text-decoration:underline;">rejected ideas</a> names the ideas that did not survive the research checks.</p>`,
