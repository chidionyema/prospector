 ...
 * comma separator, no separator with a headline, and no separator with `headline: null`.
 *
 * Counted on that capture (not assumed): 7 comma-separated, 0 em-dash, 0 en-dash, 0 spaced
 * hyphen, 8 with no separator, and of those 8, only 3 had a headline to fall back to. That last
 * group is why the fallback must tolerate a missing headline rather than assume one. (The
 * spec's Part 0 figure of "9 of 15" does not match the payload; the fixture below is the
 * payload.)
 *
 * Em-dash and en-dash are still tested in the `splitTitle separators` block below as a safety
 * net in case any historical pack slips through with the old separator.
 */
...
describe('splitTitle against the captured live titles (AC-15)', () => {
  it('exercises every title shape the catalogue has produced', () => {
    // Not a count of the catalogue, a guard that the fixture still covers all three shapes.
...
describe('splitTitle separators', () => {
  it('handles the comma separator', () => {
