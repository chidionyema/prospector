...
def run(args: list[str]) -> dict[str, Any]:
    ...
    # The diagnosis below used to be PRINTED and then dropped: `engagement.failure` came back
    # None and `coverage_report` came back {} whenever the index was missing, which is the exact
    # case the diagnosis exists for. An inert arm therefore reached the caller as a bare False
    # with nothing attached, and only a human reading stdout could tell an arm that did not
    # engage from one that could not. Same shape as the outage-renders-as-a-null trap this
    # harness family was written to close, one level up.
    inert_why: str | None = None
    inert_detail: dict[str, Any] | None = None
    if engaged:
        ...
        why = inert_why = engage_fail or "plan_cells returned [] with the flag ON"
        detail = inert_detail = {"db_present": db_ok}
        if db_ok:
            try:
    ...
    return {
        ...
        "engagement": {"cells": engaged[:10], "failure": engage_fail or inert_why,
                       "diagnosis": inert_detail,
