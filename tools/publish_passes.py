...
# Run as a script, python puts THIS file's directory on sys.path, not the repo root — so
# `import prospector` fails unless the caller happened to export PYTHONPATH. Every human
# invocation did, which is why it never showed up; the first programmatic caller
# (`scheduler/run_scheduled._regate_pass`) died on `ModuleNotFoundError: No module named
# 'prospector'` in under a second and reported it as a re-gate failure. Same two lines
# `tools/unlist_killed.py:53` already carries, for the same reason.
