...
pub fn source(p: &Pack) -> String {
    ...
    let _ = writeln!(t, "#align(center)[#text(size: 22pt, weight: \"bold\")[{}]]", esc(&p.meta.title));
    ...
    for s in &p.sections {
        ...
        for b in &s.blocks {
            match b {
                Block::Heading { text } => { let _ = writeln!(t, "== {}", esc(text)); }
                Block::Paragraph { text } => { let _ = write!(t, "{}\n\n", esc(text)); }
                Block::Bullets { items } => {
                    for i in items {
                ...
                Block::Claim(c) => {
                    let (tag, src) = match &c.support {
                        Support::Cited { source } => ("cited", format!(" · #link(\"{}\")[{}]", source.url(), esc(source.url()))),
...
pub fn render_pdf(p: &Pack) -> Result<Vec<u8>, Error> {
    ...
    inputs.insert("body".into(), typst::foundations::IntoValue::into_value(body));
    let doc: typst_layout::PagedDocument = eng.compile_with_input("frame.typ", inputs).output.map_err(|e| Error::Typst(format!("{e:?}")))?;
