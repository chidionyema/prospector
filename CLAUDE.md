...
- **errors.py / health.py** — the failover classifier and the persisted dead-mark store. `classify_exhaustion` (`errors.py:120`) separates TRANSIENT backpressure (429/503/529, `overloaded_error`) from PERMANENT exhaustion (402, credit balance, usage limit); PERMANENT wins ties, and HTTP codes are matched on word boundaries so a request id or byte count cannot bench a live brain. Transient failures get `TRANSIENT_EXHAUSTION_S` = 60s (`health.py:54`), permanent ones the 1h default. `_claim_probe` (`health.py:130`) makes the dead mark half-open: one caller — exactly one, machine-wide, since the claim is persisted — is let through on a geometric backoff to re-probe, so a brain that recovers in 90s is back in 90s rather than at the end of its window.
