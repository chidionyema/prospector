...
**Use web tools for grounding:** the retrieval chain is `[ddg, exa, claude_cli]` (`config.yaml retrieval.provider`) — free DuckDuckGo, then Exa, then Claude Code's own web search as the always-available backstop. Gemini is gone (no `gemini` key in `config.yaml`). Verdicts are ruled FINALLY only by `MOAT_PRIMARY = {claude_cli, claude}` (`prospector/operator.py:1068`); cursor_cli was deleted 2026-08-06. MiniMax is BACK on the verdict chain as a guardrailed emergency tail (`config.yaml:53 operator: [claude_cli, standardcompute, minimax]` — verified on disk 2026-08-10; standardcompute joined the middle after the 2026-08-08 directive "veridt should also have fallback", which reversed the 2026-08-06 removal), so losing claude_cli now yields a `provisional` ruling instead of a DEFER. The chain must stay LED by a trusted brain — the tail is failover, never the normal path. `is_provisional_provider` (`operator.py:1071`) is what makes that safe: anything outside MOAT_PRIMARY that rules is stamped `provisional`, never publishes on PASS (`run.py:567`), and is auto re-vetted. DeepSeek remains non-critical generation and triage ONLY.
...
- **operator.py** — swappable brain (Claude CLI/API, DeepSeek, MiniMax, Ollama, Mock). `MOAT_PRIMARY` (`:885`) is the only set that may rule FINALLY; MiniMax also sits at the tail of the verdict chain from 2026-08-08, and DeepSeek/MiniMax both sit in tiered non-critical chains for generation, prescreen and scoring. Anything of theirs that rules is stamped `provisional` and re-vetted, never finalised. `_build_operator` raises `ValueError` for the removed `cursor_cli`, so a stale config or plist fails loudly at startup instead of silently building a shorter chain.
...
- **run.py** — CLI entry point; orchestrates RUN.md's eight steps. Builds the tiered non-critical chain for generation/prescreen/score: `_noncritical_order(cfg)` (`run.py:320`) reads `config.yaml:70 noncritical_operator:`, falling back to the hardcoded default `_NONCRITICAL_ORDER = ("standardcompute", "claude_cli", "minimax")` (`run.py:317`). Handles moat exhaustion with DEFER + `vet --resume`; persists failed signals to `signals/pending/` for `generate --resume`.
